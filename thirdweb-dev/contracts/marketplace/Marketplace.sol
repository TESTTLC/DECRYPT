  ...
  IERC1155ReceiverUpgradeable
{
  ...
  function validateERC20BalAndAllowance(
    ...
    string memory result = string(
      bytes.concat(
        bytes.concat('insufficient balance or allowance - .'),
        ' - _addrToCheck:',
        bytes(abi.encodePacked(_addrToCheck)),
        ' - _currency: ',
        bytes(abi.encodePacked(_currency)),
        ' - _currencyAmountToCheckAgainst ',
        bytes(abi.encodePacked(_currencyAmountToCheckAgainst)),
        ' -  IERC20Upgradeable(_currency).balanceOf(_addrToCheck): ',
        bytes(
          abi.encodePacked(IERC20Upgradeable(_currency).balanceOf(_addrToCheck))
        ),
